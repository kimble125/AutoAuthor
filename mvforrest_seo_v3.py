...
def get_google_suggestions(keyword: str) -> list[str]:
    """
    구글 자동완성 API로 연관 검색어를 수집합니다.
    자동완성에 등장 = 실제 검색 수요가 있는 키워드입니다.
    """
    url = "https://suggestqueries.google.com/complete/search"
    params = {"q": keyword, "client": "firefox", "hl": "ko", "gl": "KR"}
    headers = {
        "User-Agent": (
    ...
    try:
        ...
        print(f"  ⚠️  구글 자동완성 오류 ({keyword}): {e}")
        return []
...
def generate_blog_titles_with_gemini(
    ...
    cross_topics: list[tuple[str, str]] | None = None,
) -> str:
    """
...
6. 각 제목은 서로 다른 황금 키워드를 중심으로 작성 (중복 키워드 사용 최소화)

[출력 형식 — 반드시 아래 형식을 정확히 따르세요]
제목 1: [제목 전문]
→ 핵심 키워드: [사용한 황금 키워드] | 검색 의도: [정보형/탐색형/거래형] | 예상 클릭률: [상/중/하]
...
def save_results(
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in results:
            writer.writerow({
                "키워드": r["keyword"],
                "수요(구글자동완성)": "O" if r["has_demand"] else "X",
